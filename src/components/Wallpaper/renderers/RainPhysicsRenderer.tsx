  ...
  const material = useMemo(() => {
    const shader = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        vec2 Drops(vec2 uv, float t, float l1, float l2) {
        ...
        vec2 MicroDrops(vec2 uv) {
          // Micro-droplet "sparkle" layer: static tiny drops that read as glass
          // highlights, without strong refraction (which can look like colored pixels
          ...
          float density = step(.35, n.z); // keep most cells "on"
          vec2 p = (n.xy - .5) * .65;
          float r = mix(.06, .16, n.x);
          float d = length(st - p);

          float mask = S(r, 0., d) * density;
          float rim = S(r, r * .65, d) - S(r * .65, r * .35, d);
          rim = max(0., rim) * density;
        ...
        void main() {
          ...
          vec2 c = Drops(uv, t, layer1, layer2);

          vec2 e = vec2(.001, 0.);
          float cx = Drops(uv + e, t, layer1, layer2).x;
          float cy = Drops(uv + e.yx, t, layer1, layer2).x;
          vec2 n = vec2(cx - c.x, cy - c.x);

          float focus = mix(maxBlur - c.y, minBlur, S(.1, .2, c.x));
          vec3 col = texLod(iChannel0, UV + n, focus).rgb;

          vec2 micro = MicroDrops(uv);
          float microMask = micro.x;
          float microRim = micro.y;

          vec2 me = vec2(.0015, 0.);
          float mx = MicroDrops(uv + me).x;
          float my = MicroDrops(uv + me.yx).x;
          vec2 mn = vec2(mx - microMask, my - microMask);

          vec3 microNormal = normalize(vec3(mn * 18., 1.));
          vec3 lightDir = normalize(vec3(-.35, .55, 1.));
          float mndl = clamp(dot(microNormal, lightDir), 0., 1.);
          float microSpec = pow(mndl, 26.);

          // Slight dimming + rim/spec highlight to read as glass "beads"
          col *= 1. - microMask * .04;
          col += (microRim * .05 + microSpec * .06) * microMask;
